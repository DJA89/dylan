...
async function main() {
  ...
  discography = discography.split('\n').filter((disk) => disk).map((disk) => (disk.match(/^(\d*) (.*)$/).slice(1, 3))).sort(sorter);
...
//console.log(response.request);
  ...
  const albums_images  = [...first_albums_images, ...second_albums_images].map((disk) => ([disk.release_date.substring(0, 4), disk.name, disk.images])).sort(sorter);
